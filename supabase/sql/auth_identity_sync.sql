...
begin
  ...
  if to_regclass('public.krakens_roster') is not null then
    execute 'alter table public.krakens_roster add column if not exists "AUTH_USER_ID" text';
  end if;
  if to_regclass('public.warriors_roster') is not null then
    execute 'alter table public.warriors_roster add column if not exists "AUTH_USER_ID" text';
  end if;
  if to_regclass('public.knights_roster') is not null then
    execute 'alter table public.knights_roster add column if not exists "AUTH_USER_ID" text';
  end if;
end $$;
...
begin
  ...
  if to_regclass('public.krakens_roster') is not null then
    execute $sql$
      update public.krakens_roster r
      set "AUTH_USER_ID" = u.id::text
      from auth.users u
      where lower(coalesce(r."EMAIL", '')) = lower(coalesce(u.email, ''))
        and coalesce(r."EMAIL", '') <> ''
  ...
  if to_regclass('public.warriors_roster') is not null then
    execute $sql$
      update public.warriors_roster r
      set "AUTH_USER_ID" = u.id::text
      from auth.users u
      where lower(coalesce(r."EMAIL", '')) = lower(coalesce(u.email, ''))
        and coalesce(r."EMAIL", '') <> ''
  ...
  if to_regclass('public.knights_roster') is not null then
    execute $sql$
      update public.knights_roster r
      set "AUTH_USER_ID" = u.id::text
      from auth.users u
      where lower(coalesce(r."EMAIL", '')) = lower(coalesce(u.email, ''))
        and coalesce(r."EMAIL", '') <> ''
